...
function HeroSection({ featuredMovie }: { featuredMovie: Movie | null }) {
  ...
  return (
    <div className="relative h-[70vh] overflow-hidden rounded-xl">
      <Image
        src={api.utils.getTmdbImageUrl(featuredMovie?.backdropPath || "", "original")}
      ...
      <div className="absolute bottom-0 left-0 p-8 max-w-2xl">
        <div className="space-y-4 animate-in slide-in-from-bottom-4 duration-1000">
          ...
          <h1 className="text-5xl font-bold text-white mb-4">{featuredMovie.title}</h1>
          ...
          <p className="text-gray-300 text-lg leading-relaxed mb-6 line-clamp-3">{featuredMovie.overview}</p>
...
function MediaCard({ item, type }: { item: Movie | TvSeries; type: "movie" | "series" }) {
      ...
      <Card className="group bg-gray-900/50 border-gray-800 hover:border-gray-600 transition-all duration-300 hover:scale-105 cursor-pointer">
        ...
        <CardContent className="p-4">
          ...
          <div className="flex items-center justify-between text-sm text-gray-400">
            <span>{isMovie ? movie.year : new Date(series.firstAirDate || "").getFullYear()}</span>
          ...
          <section className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
              {searchResults.map((item) => (
                <MediaCard key={item.id} item={item} type={"episodes" in item ? "series" : "movie"} />
              ))}
            </div>
          </section>
        )}
            ...
            <CardContent className="p-6 text-center">
              ...
              <div className="text-2xl font-bold text-white">
                {featuredMovies.length > 0
                  ? (featuredMovies.reduce((sum, m) => sum + (m.rating || 0), 0) / featuredMovies.length).toFixed(1)
                  : "0.0"}
              </div>
