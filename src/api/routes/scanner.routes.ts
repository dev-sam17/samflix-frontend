...
const scanHandler: AsyncRouteHandler = async (_req, res) => {
  try {
    ...
    res.write(
      `data: ${JSON.stringify({ status: "Starting scan", progress: 0 })}\n\n`
    );
...
const addFolderHandler: AsyncRouteHandler<{}, any, FolderBody> = async (
  req,
  res
) => {
  try {
...
// Delete media folder
const deleteFolderHandler: AsyncRouteHandler<{ id: string }> = async (
  req,
  res
) => {
  try {
...
> = async (req, res) => {
  try {
    ...
    const conflict = await scannerService.resolveConflict(
      req.params.id,
      selectedId
    );
    res.json(conflict);
...
const deleteConflictHandler: AsyncRouteHandler<{ id: string }> = async (
  req,
  res
) => {
  try {
